...
lower standard error for feeling temperature and more importantly a higher 
correlation coefficient and F-statistic. 

## Q9
```{r q9}
# fit full model
# names(d)
lm_full <- lm(cnt ~ season + years + hbin + workbin + weathersit + raw_temp + raw_ftemp + raw_hum + raw_ws + (raw_ftemp * hbin), d)
summary(lm_full)
```
