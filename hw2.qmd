...
d <- dcbikeshare
as.factor(d$season)
(d$season <- mapvalues(d$season, from = c('1','2','3', '4'), to = c("Winter", "Spring", "Summer", "Fall"))) # assigns strings to number values 

d$season <- fct_relevel(d$season, c('Spring', 'Summer', 'Fall', 'Winter')) #relevels with spring as baseline
levels(d$season)
```
...
summary(lm2) 
```
Feeling temperature is a better predictor of bike rentals, although not by large amount. Both have a statistically significant result (p < .001), but there is a lower standard error for feeling temperature and more importantly a higher correlation coefficient and F-statistic. 

## Q9
```{r q9}
# fit full model
names(d)
lm_full <- lm(cnt ~ season + years + hbin + workbin + weathersit + raw_temp + raw_ftemp + raw_hum + raw_ws + (raw_ftemp * holiday), d)
summary(lm_full)
```
